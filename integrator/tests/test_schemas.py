...
class TestERPProductValidation:
    def test_valid_product(self):
        p = ERPProduct(
            id="SKU-001", title="Test", price_vat_excl=100.0, stocks={"a": 1}
        )
        assert p.id == "SKU-001"
    ...
    def test_null_price_not_allowed(self):
        with pytest.raises(ValidationError):
            _ = ERPProduct(
                id="SKU-004", title="NullPrice", price_vat_excl=None, stocks={}
            )

    def test_null_attributes_allowed(self):
        p = ERPProduct(
            id="SKU-003", title="NoAttr", price_vat_excl=100, attributes=None
        )
...
class TestEshopProductTransformation:
    ...
    def test_stock_summation(self):
        erp = ERPProduct(
            id="SKU-001", title="Item", price_vat_excl=100, stocks={"a": 5, "b": 3}
        )
        eshop = EshopProduct.from_erp(erp)
    ...
    def test_stock_ignores_non_numeric(self):
        erp = ERPProduct(
            id="SKU-008", title="Filtry", price_vat_excl=300, stocks={"a": "N/A"}
        )
        eshop = EshopProduct.from_erp(erp)
    ...
    def test_stock_mixed_numeric_and_non_numeric(self):
        erp = ERPProduct(
            id="X", title="X", price_vat_excl=10, stocks={"a": 5, "b": "N/A", "c": 3}
        )
        eshop = EshopProduct.from_erp(erp)
        assert eshop.stock_total == 8
    ...
    def test_color_default_when_missing(self):
        erp = ERPProduct(
            id="SKU-006", title="Tablety", price_vat_excl=250, attributes={}
        )
        eshop = EshopProduct.from_erp(erp)
    ...
    def test_color_default_when_null_attributes(self):
        erp = ERPProduct(
            id="SKU-003", title="Mlýnek", price_vat_excl=1500, attributes=None
        )
        eshop = EshopProduct.from_erp(erp)
        assert eshop.attributes["color"] == "N/A"
    ...
    def test_compute_hash_changes_on_different_data(self):
        ...
        assert (
            EshopProduct.from_erp(e1).compute_hash()
            != EshopProduct.from_erp(e2).compute_hash()
        )
